...
class EMDRegularizedLoss(nn.Module):
    ...
    def __init__(self, reduction: str = 'mean', alpha_: float = 0.5, eps: float = 1e-20):
    ...
    def forward(self, outputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        loss = self.alpha_ * loss1 + (1 - self.alpha_) * loss2    
...
class L1_EMDRegularizedLoss(nn.Module):
    ...
    def __init__(self, reduction: str = 'mean', alpha_: float = 0.5, beta_:float = 0.3, eps: float = 1e-20):
