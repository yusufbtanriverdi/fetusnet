...
class ResUNet3D(nn.Module):
    def __init__(self, input_channels, output_channels, base_features, coordinate_regression=False):
        ...
        self.encoder_block4 = ResDown(8 * base_features, 16 * base_features)
        self.encoder_block5 = ResDown(16 * base_features, 32 * base_features)
        self.bottleneck = ResDown(32 * base_features, 64 * base_features)

        # Decoder blocks with upsampling
        self.decoder_block1 = ResUp(64 * base_features, 32 * base_features)
        self.decoder_block2 = ResUp(32 * base_features, 16 * base_features)
        self.decoder_block3 = ResUp(16 * base_features, 8 * base_features)
        self.decoder_block4 = ResUp(8 * base_features, 4 * base_features)
        self.decoder_block5 = ResUp(4 * base_features, 2 * base_features)
        self.decoder_block6 = ResUp(2 * base_features, base_features)
        ...
        if coordinate_regression:
            ...
            self.fc = nn.Linear(2048, 3 * output_channels)
    ...
    def forward(self, x):
        ...
        x_enc4 = self.encoder_block4(x_enc3)        # Downsample to 16x base_features
        x_enc5 = self.encoder_block5(x_enc4)        # Downsample to 32x base_features   
        bottleneck_output = self.bottleneck(x_enc5) # Bottom layer at 16x base_features

        # Decoder path with skip connections
        x_dec1 = self.decoder_block1(bottleneck_output, x_enc5)
        x_dec2 = self.decoder_block2(x_dec1, x_enc4)
        x_dec3 = self.decoder_block3(x_dec2, x_enc3)
        x_dec4 = self.decoder_block4(x_dec3, x_enc2)
        x_dec4 = self.decoder_block4(x_dec4, x_enc1)
        x_dec5 = self.decoder_block5(x_dec5, x_initial)
        # Output layer
        output = self.output_layer(x_dec5)
